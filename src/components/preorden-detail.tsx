...
export type PreordenDetailProps = {
  id: string;
  from?: "reportes" | "panel" | "preordenes" | "solicitudes";
  status?: string;
  onClose?: () => void;
  showBackLink?: boolean;
};

export function PreordenDetail({ id, from, status, onClose, showBackLink = true }: PreordenDetailProps) {
  ...
  function applyModify(idx: number, pct: number) {
    setLines((ls) => ls.map((l, i) => (i === idx ? { ...l, status: "Modificada", authorizedPct: pct, requiresMyAction: false } : l)));
  ...
  const backDest = from === "reportes" ? "/panel/reportes" : from === "panel" ? "/panel" : from === "solicitudes" ? "/panel/solicitudes" : "/panel/preordenes";
  const backLabel = from === "reportes" ? "Reportes" : from === "panel" ? "Dashboard" : from === "solicitudes" ? "Solicitudes" : "Preórdenes";

  function handleBack() {
    if (onClose) onClose();
    else navigate({ to: backDest });
  }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          {showBackLink && (
            ...
            >
              <ArrowLeft className="h-3.5 w-3.5" /> Volver a {backLabel}
            </button>
          )}
          ...
          <div className="space-y-2">
            <button
              ...
              onClick={() => {
                ...
                setTrace((t) => [
                  ...t,
                  { at: new Date().toLocaleString("es-CO"), who: "Moises Carmona", action: "Gestión confirmada" },
                ]);
                handleBack();
              }}
            ...
            {closed && (
              <p className="text-center text-[11px] text-muted-foreground">
                {readOnly ? "Esta solicitud está en modo solo lectura." : "Todas las líneas ya fueron gestionadas."}
              </p>
            )}
          </div>
        );
      ...
      {modal && (
        <DecisionModal
          ...
          onModify={(pct) => { applyModify(modal.idx, pct); setModal(null); }}
          onReject={(reason) => { applyReject(modal.idx, reason); setModal(null); }}
          onCancel={(reason) => { applyCancel(modal.idx, reason); setModal(null); }}
        />
      )}
...
function ProductStatusBadge({ status }: { status: ProductStatus }) {
  ...
  return <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-medium ${map[status]}`}>{status}</span>;
}
...
function ProviderBadge({ status, pct }: { status: ProviderStatus; pct?: number }) {
  ...
  return <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-medium ${map[status]}`}>{status}{showPct ? ` · ${pct}%` : ""}</span>;
}
...
function DecisionModal({
  kind, line, onClose, onModify, onReject, onCancel,
}: {
  ...
  const titles = { modify: "Modificar porcentaje", reject: "Rechazar línea", cancel: "Cancelar línea" } as const;

  function submit() {
    setErr(null);
    if (kind === "modify") {
      if (!/^\d{1,3}(\.\d)?$/.test(pct)) { setErr("Ingresa un valor entre 0 y 100 con máximo un decimal."); return; }
      const n = Number(pct);
      if (!(n >= 0 && n <= 100)) { setErr("El porcentaje debe estar entre 0 y 100."); return; }
      onModify(n);
    } else if (kind === "reject") {
      if (reason.trim().length < 5) { setErr("La justificación es obligatoria (mínimo 5 caracteres)."); return; }
      onReject(reason.trim());
    } else {
      if (reason.trim().length < 5) { setErr("Indica el motivo de la cancelación."); return; }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4 animate-fade-in" onClick={onClose}>
      <div className="w-full max-w-md rounded-xl border border-border bg-card p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
          ...
          {kind === "modify" ? (
            <div>
              ...
              <div className="mt-1 flex items-center gap-2">
                <input type="text" inputMode="decimal" value={pct} onChange={(e) => setPct(e.target.value)} placeholder="Ej. 7.5"
            ...
            <div>
              ...
              <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={4} placeholder="Describe el motivo…"
        ...
        <div className="mt-5 flex items-center justify-end gap-2">
          <button onClick={onClose} className="rounded-lg border border-border bg-background px-3.5 py-2 text-xs font-medium hover:bg-accent">Cancelar</button>
          <button onClick={submit} className="rounded-lg bg-primary px-3.5 py-2 text-xs font-medium text-primary-foreground hover:opacity-90">Confirmar</button>
        </div>
      </div>
    </div>
  );
