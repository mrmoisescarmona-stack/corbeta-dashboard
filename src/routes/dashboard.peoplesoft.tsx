...
const dispatches = [
  ...
  { id: "PR-2024-000239", status: "Error", at: "28/05/2024 09:42", attempts: 3 },
  { id: "PR-2024-000238", status: "Reintentando", at: "28/05/2024 09:21", attempts: 2 },
];
      ...
      {showRoot ? (
        <div className="rounded-xl border border-border bg-card">
          <div className="flex items-center gap-2.5 p-5 border-b border-border">
            ...
            <h3 className="text-base font-semibold">Cola de envío</h3>
          </div>
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[11px] uppercase tracking-wider text-muted-foreground">
                ...
                <th className="font-medium px-5 py-3 text-right">Última actividad</th>
              </tr>
            </thead>
            <tbody>
              {dispatches.map((d) => (
                <tr key={d.id} className="border-t border-border hover:bg-muted/40">
                  ...
                  <td className="px-5 py-3.5 text-right text-muted-foreground tabular-nums">{d.at}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function DispatchBadge({ status }: { status: string }) {
  ...
  const { tone, Icon } = map[status] ?? map["Enviada"];
  return (
