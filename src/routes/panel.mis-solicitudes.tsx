...
export const Route = createFileRoute("/panel/mis-solicitudes")({
  head: () => ({ meta: [{ title: "Mis Solicitudes · Corbeta" }] }),
...
const CURRENT_APPROVER = { name: "Juan Sebastián Rodríguez", initials: "JR" };

const rows: Row[] = Array.from({ length: 8 }).map((_, i) => {
  ...
  return {
    ...
    linesTotal,
    linesApproved,
    deadlineHours: [-2, 4, 26, 72, -5, 8, 30, 12][i],
...
function MisSolicitudesPage() {
  ...
  const stats = useMemo(() => {
    const pendientes = rows.filter((r) => r.status === "Pendiente" || r.status === "Espera proveedor").length;
    const aprobadas = rows.filter((r) => r.status === "Aprobada" || r.status === "Enviada a PeopleSoft").length;
    const rechazadas = rows.filter((r) => r.status === "Rechazada").length;
    const valor = rows.reduce((s, r) => s + r.valueM, 0);
    return [
      { label: "Asignadas a mí", value: rows.length, delta: "Total" },
      { label: "Pendientes", value: pendientes, delta: "Requieren acción" },
      { label: "Aprobadas", value: aprobadas, delta: "" },
      { label: `Valor · $${valor.toFixed(1)}M`, value: rechazadas, delta: "Rechazadas" },
    ];
  }, []);
  ...
  const heading = isProveedor ? "Preórdenes para tu respuesta" : "Preórdenes asignadas a mí";
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <span className="inline-flex items-center rounded-full bg-primary/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-primary">Mis Solicitudes</span>
          <h2 className="mt-2 text-2xl md:text-3xl font-semibold tracking-tight">{heading}</h2>
      ...
      <div className="rounded-xl border border-border bg-card">
        <div className="flex items-center justify-between p-5 border-b border-border">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="text-base font-semibold">Mis preordenes</h3>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2 p-5 border-b border-border">
          ...
          <div className="relative">
            <button
              onClick={() => { setShowStatusMenu((v) => !v); setShowDeadlineMenu(false); }}
          ...
          <div className="relative">
            <button
              onClick={() => { setShowDeadlineMenu((v) => !v); setShowStatusMenu(false); }}
              className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-background px-3 py-2 text-xs font-medium hover:bg-accent"
            >
        ...
        <div className="flex items-center justify-between p-5 border-t border-border">
          <span className="text-xs text-muted-foreground">Mostrando {filtered.length} de {rows.length}</span>
